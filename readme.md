...
Чёткого порядка нет, они стартуют почти одновременно.
Но по логике:
...
Контейнеры связаны через сеть и работают вместе как одно приложение.

В целом стало понятно, как строятся реальные проекты на Docker.